...
3. The server returns a bounded Supabase database batch for every player.
...
- A guest can play Arcade and Training from readable Supabase database content,
  receives a complete local summary, and is never shown a false save
  confirmation.

...
Phase one includes a playable, authenticated Next.js application; a generic
challenge engine; a normalized Supabase backend; and a lightweight,
redistribution-safe starter corpus.
...
- Supabase email/password authentication, protected routes, PostgreSQL, RLS,
  RPC-based attempt persistence, and optional Storage uploads
- One binary game engine with registered image, email, and audio renderers
- Configurable scoring, difficulty, XP, animation, UI, category, and round rules
- Zustand for transient round state and Zod at data boundaries
- Arcade gameplay plus Training, Analytics, Profile, and Settings shell routes
...
- Vitest unit tests and a Playwright smoke-test foundation
...
4. Apply the database migration. With a linked Supabase CLI project:
...
flows also require a configured project and a dedicated test account; the
committed smoke test only covers the public landing page.
...
supabase/migrations/     Idempotent normalized schema, functions, and RLS
...
Phase one does not ship multiplayer, public leaderboards, classroom management,
video challenges, or a production analytics pipeline. The schema and renderer
registry leave explicit extension points for those features. Cloud migration,
hosted seeding, Storage uploads, and authenticated browser testing require
project credentials and are never reported as successful unless run against a
